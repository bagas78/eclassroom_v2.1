        ...
        <div class="box-body">
          ...
          <table id="example1" class="table table-bordered table-hover">
                ...
                <?php foreach ($data as $key): ?>
                  ...
                  <?php $tgl = date_create($key['post_pelaksanaan']); ?>
                                  
                  <tr>
                    <td><?php echo $key['post_judul'] ?></td>
                    <td>Individu</td>
                    <td><?php $str = str_replace(['[',']'], '', $key['post_kelas']); $db = $this->db->query("SELECT kelas_nama FROM t_kelas WHERE kelas_id IN($str)")->result_array(); foreach ($db as $val) {echo '<span class="badge">'.$val['kelas_nama'].'</span> ';} ?></td>
                    <td>
                      <div style="width: 80px;">

                        <?php if ($this->session->userdata('level') == 3): ?>
                          
                          <a href="<?php echo base_url('post/kerjakan/'.$key['post_id']) ?>"><button class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button></a>

                        <?php endif ?>

                        <?php if ($this->session->userdata('level') < 3): ?>
                          
                          <a href="<?php echo base_url('post/edit/').$key['post_id'] ?>"><button class="btn btn-xs btn-default"><i class="fa fa-edit"></i></button></a>

                          <button onclick="del('<?php echo base_url() ?>post/delete/<?php echo $key['post_id'] ?>')" class="btn btn-xs btn-default"><i class="fa fa-trash"></i></button>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <form role="form" method="post" action="<?php echo base_url() ?>post/add" enctype="multipart/form-data">
                  <div class="box-body">
                    ...
                    <div class="form-group">
                      <label>Jumlah</label>
                      <input required="" type="text" name="post_jumlah" class="form-control">
                    </div>

                    <div class="form-group">
                      <label>Kelas </label>
                      <select name="post_kelas[]" class="form-control select2" multiple="multiple" data-placeholder="-- Pilih --" style="width: 100%;">
