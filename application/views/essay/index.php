          ...
          <table id="example1" class="table table-bordered table-hover">
                ...
                <?php foreach ($data as $key): ?>
                  ...
                  <tr>
                    ...
                    <td><?php echo $key['essay_jumlah'] ?></td>
                    <td><?php echo $key['pelajaran_nama'] ?></td>

                    <?php if ($this->session->userdata('level') == 3): ?>
                      <td>

                        <?php $soal = $key['essay_id'] ?>
                        <?php $iduser = $this->session->userdata('id'); ?>
                        <?php $count = $this->query_builder->view_row("SELECT * FROM t_essay_hasil WHERE essay_hasil_soal = '$soal' AND essay_hasil_siswa = '$iduser'"); ?>
                    ...
                    <td style="width: 80px;">
                      ...
                      <?php if ($this->session->userdata('level') > 2): ?>
                        <a href="<?php echo base_url('essay/kerjakan/'.$key['essay_id']) ?>"><button class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></button></a>
                      <?php else: ?>
                         <a href="<?php echo base_url('essay/edit/'.$key['essay_id']) ?>"><button class="btn btn-xs btn-default"><i class="fa fa-edit"></i></button></a>
                      <button onclick="del('<?php echo base_url() ?>essay/delete/<?php echo $key['essay_id'] ?>')" class="btn btn-xs btn-default"><i class="fa fa-trash"></i></button>
                      <?php endif ?>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">Tambah Essay</h4>
          </div>
          <div class="modal-body">
            <form role="form" method="post" action="<?php echo base_url() ?>essay/add" enctype="multipart/form-data">
              <div class="box-body">
                <div class="form-group">
                  <label>Judul</label>
                  <input required="" type="text" name="judul" class="form-control" placeholder="Nama Soal" value="">
                </div>
                <div class="form-group">
                  <label>Jumlah</label>
